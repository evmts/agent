...
- [ ] Spawn subagent to verify LSP server binaries are available on system
- [ ] Spawn subagent to test LSP initialization with a real server
- [ ] Spawn subagent to verify hover requests return expected data
- [ ] Spawn subagent to check JSON-RPC protocol compliance
- [ ] Spawn subagent to run integration tests with multiple language servers
- [ ] Spawn subagent to verify thread safety and concurrent requests
- [ ] Spawn subagent to check for resource leaks (unclosed processes/pipes)
...
<criteria>
- [ ] LSP client can initialize and communicate with language servers over stdio
- [ ] Hover tool returns type information for Python code
- [ ] Hover tool returns type information for TypeScript/JavaScript code
- [ ] Hover tool returns type information for Go code
- [ ] Multiple concurrent hover requests are handled correctly
- [ ] File versions are tracked properly across didOpen/didChange notifications
- [ ] Manager reuses existing clients for same workspace root
- [ ] Server crashes are detected and marked to prevent retry loops
- [ ] Hover responses are parsed for all content types (string, MarkupContent, MarkedString[])
- [ ] Clear error messages when LSP server is not installed
- [ ] Graceful degradation when no LSP server available for file type
- [ ] All clients are properly shutdown on agent termination
- [ ] No process/pipe leaks after repeated hover requests
- [ ] Integration tests pass with real language servers
- [ ] Tool is registered and accessible to agent
- [ ] Documentation explains how to install required LSP servers
</criteria>
</acceptance-criteria>
...
- [ ] Start agent, send hover request to Python file
- [ ] Verify hover contents contain type information
- [ ] Check that second hover reuses existing client
- [ ] Restart LSP server, verify agent recovers
- [ ] Open file in different workspace, verify separate client spawned
- [ ] Kill LSP server process mid-request, verify graceful error
</testing>
