...
1. **Anthropic's Built-in WebFetchTool**: The plan assumed we could configure Anthropic's built-in WebFetchTool, but it only supports token-based limits (`max_content_tokens`), not byte-based size limits. This limit is also approximate and doesn't apply to binary content like PDFs.

2. **Custom Tool Implementation Required**: Instead of configuring an existing tool, we had to implement a completely custom `web_fetch` tool from scratch using httpx to enforce the 5MB byte limit properly.

3. **Tool Registration**: The custom tool was registered as an `@agent.tool_plain` decorated function in `agent/agent.py`, following the same pattern as other custom tools like browser automation and LSP tools.

### What Was Already Implemented

- Nothing related to web fetch size limits was previously implemented
- The agent already had Anthropic's built-in WebSearchTool configured, but no custom web fetch implementation
- httpx was already available as a dependency (imported in agent.py for browser_client)

### Implementation Details

**Files Created:**
1. `/Users/williamcory/agent/core/constants.py` - Constants module with MAX_RESPONSE_SIZE (5MB), DEFAULT_WEB_TIMEOUT (30s), and MAX_WEB_TIMEOUT (120s)
2. `/Users/williamcory/agent/agent/tools/web_fetch.py` - Custom web fetch implementation with size limits
3. `/Users/williamcory/agent/tests/test_web_fetch.py` - Comprehensive test suite (10 tests, all passing)

**Files Modified:**
1. `/Users/williamcory/agent/agent/agent.py` - Added import and registered `web_fetch` tool

**Key Implementation Approach:**
- Two-stage size checking: HEAD request first (if supported), then streaming GET with size enforcement
- Graceful fallback: If HEAD fails or doesn't return Content-Length, continue with GET
- Streaming read with incremental size checking to prevent full download of oversized content
- Proper error handling with clear, actionable error messages
- Character encoding detection from Content-Type header with fallback to latin-1

### Testing Challenges

The main challenge was properly mocking httpx's async context managers in tests. Initial attempts failed because:
- AsyncMock's `stream()` method returned a coroutine instead of a context manager
- Had to use a custom helper function that creates Mock objects with proper `__aenter__` and `__aexit__` async methods
- Header dictionaries needed to be regular dicts, not Mocks, to avoid type errors

### Useful Context For Future Implementation

1. **Custom Tools Pattern**: When Anthropic's built-in tools don't support the required functionality, implement custom tools using `@agent.tool_plain` decorator in `agent/agent.py`

2. **Constants Best Practice**: All magic numbers should be defined as named constants in `core/constants.py` (or appropriate module) in SCREAMING_CASE, following the project's style guide

3. **httpx Async Patterns**: When using httpx for async HTTP requests:
   - Use `httpx.AsyncClient()` as an async context manager
   - Use `client.stream()` for streaming downloads with size limits
   - Check Content-Length header before downloading when possible
   - Implement graceful fallbacks for servers that don't support HEAD or Content-Length

4. **Test Mocking**: When mocking httpx async operations:
   - Use Mock (not AsyncMock) for the client to avoid coroutine issues
   - Create async functions for async methods like `head()`
   - Create proper MagicMock objects with `__aenter__` and `__aexit__` for context managers
   - Use regular dicts for headers, not Mock objects

5. **Error Handling**: The implementation follows a defensive pattern:
   - Try HEAD first, catch and continue on failure
   - Multiple layers of size checking (header check, streaming check, final validation)
   - Clear error messages that explain the limit and suggest alternatives
