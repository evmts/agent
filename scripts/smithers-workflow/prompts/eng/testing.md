...
`swift test` or `zig build unit-test`. No Xcode project needed.
...
**SmithersServicesTests:** JJService output parsing (mock strings); CommitStyleDetector; SkillScanner (mock fs); ChatHistoryStore round-trip; JSONRPCTransport framing (mock pipes); SmithersCtlInterpreter parsing
...
`zig build ui-test`. Requires Xcode project+UI test target.
...
**Run:** `zig build playwright` (start HTTP server → run tests → teardown)
...
2. Playwright e2e (`zig build playwright`) — full stack via web
3. XCUITest (`zig build ui-test`) — native macOS UI
...
- **MockCodexTransport** — Same interface as `JSONRPCTransport`, reads recorded sessions. Records = JSON `{request, response, delay}` arrays.
- **Recording** — `SMITHERS_RECORD_CODEX=1` env → `JSONRPCTransport` writes session file during normal op.
- **Replay** — `MockCodexTransport` plays back w/ realistic timing (10x compressed). Used in unit+XCUITests.
- **Stub** — Simple tests: `StubCodexService` returns canned responses for specific calls.
...
- **Swift:** `zig build xcode-test` — macOS runner
- **Playwright e2e:** `zig build playwright` — HTTP server + real libsmithers
- **Native UI:** `zig build ui-test` — XCUITest, macOS runner
- **Web build:** `zig build web` — compile check
