...
// MARK: - Terminal FFI Stubs
// These are placeholder stubs for Zig FFI functions that will be connected later
...
enum TerminalError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .startFailed:
            return "Terminal start failed"
        case .invalidFileDescriptor:
            return "Invalid file descriptor"
        case .notConfigured:
            return "Terminal not configured - Zig backend not connected"
        }
    }
}

// Stub FFI functions - will be replaced with actual Zig FFI later
func terminal_init() -> Int32 {
    // Stub: return failure since Zig backend not connected
    return -1
}
