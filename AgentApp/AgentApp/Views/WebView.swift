...
struct WebViewRepresentable: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // Set up user content controller for browser automation
        let contentController = configuration.userContentController

        // Register message handler for automation
        contentController.add(context.coordinator, name: BrowserAutomationHandler.handlerName)

        // Inject automation script at document start
        let automationScript = WKUserScript(
            source: PlueAutomationJS.source,
            injectionTime: .atDocumentStart,
            forMainFrameOnly: false
        )
        contentController.addUserScript(automationScript)
        ...
        // Connect to browser automation manager
        Task {
            await BrowserAutomationManager.shared.connect(to: webView)
        }
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
        ...
        func userContentController(
            ...
            guard message.name == BrowserAutomationHandler.handlerName else { return }

            guard let body = message.body as? [String: Any],
                  let browserMessage = BrowserMessage(from: body) else {
                print("[WebView] Invalid message format: \(message.body)")
                return
            }

            // Forward to automation manager
            Task {
                await handleBrowserMessage(browserMessage)
            }
        }

        private func handleBrowserMessage(_ message: BrowserMessage) async {
            switch message.type {
            case .commandResult, .commandError:
                // Parse result and forward to automation actor
                let success = message.type == .commandResult
                ...
                let result = BrowserResult(
                    ...
                    data: resultData,
                    error: success ? nil : (message.payload["error"] as? String ?? "Unknown error"),
                    duration: duration
                )

                await BrowserAutomationManager.shared.automation.handleResult(result)

            case .ready:
                await BrowserAutomationManager.shared.automation.handleScriptReady()

            case .navigationStarted:
                let url = message.payload["url"] as? String ?? ""
                await BrowserAutomationManager.shared.automation.handleNavigationStarted(url: url)

            case .navigationComplete:
                let url = message.payload["url"] as? String ?? ""
                let title = message.payload["title"] as? String ?? ""
                await BrowserAutomationManager.shared.automation.handleNavigationComplete(url: url, title: title)
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            // Notify about navigation complete
