...
export fn macos_pty_init() c_int {
    if (pty_state.initialized) return 0;
    
    pty_state.initialized = true;
    std.log.info("macOS PTY initialized", .{});
    return 0;
}

/// Start the PTY with a shell
export fn macos_pty_start() c_int {
...
export fn macos_pty_write(data: [*]const u8, len: usize) isize {
    pty_mutex.lock();
...
export fn macos_pty_read(buffer: [*]u8, buffer_len: usize) isize {
    pty_mutex.lock();
    defer pty_mutex.unlock();
...
export fn macos_pty_send_text(text: [*:0]const u8) void {
    const len = std.mem.len(text);
    _ = macos_pty_write(@ptrCast(text), len);
}

/// Get the master file descriptor
export fn macos_pty_get_fd() c_int {
    if (pty_state.master_fd) |fd| {
        return @intCast(fd);
    }
    return -1;
...
/// Resize the PTY
export fn macos_pty_resize(cols: u16, rows: u16) void {
...
export fn macos_pty_deinit() void {
    macos_pty_stop();
    pty_state = PtyState{};
    _ = gpa.deinit();
}