...
const RateLimitEntry = struct {
    count: u32,
    window_start: i64,
    last_access: i64, // Track last access for TTL cleanup
};
...
/// Removes entries that haven't been accessed in TTL_CLEANUP_SECONDS
/// This should be called periodically (e.g., from a background service)
const TTL_CLEANUP_SECONDS: i64 = 3600; // 1 hour TTL for unused entries

pub fn cleanupInMemoryStore(allocator: std.mem.Allocator) void {
    store_mutex.lock();
    defer store_mutex.unlock();

    if (rate_limit_store == null) return;
    ...
    while (iter.next()) |entry| {
        const entry_age = now - entry.value_ptr.last_access;
        if (entry_age >= TTL_CLEANUP_SECONDS) {
            to_remove.append(allocator, entry.key_ptr.*) catch continue;
        }
    }

    // Remove stale entries and free their keys
    for (to_remove.items) |key| {
        if (store.fetchRemove(key)) |entry| {
            allocator.free(entry.key);
        }
    }

    if (to_remove.items.len > 0) {
        log.debug("Cleaned up {d} stale in-memory rate limit entries", .{to_remove.items.len});
    }
}

/// Cleanup expired rate limit entries in database
/// This should be called periodically (e.g., from a background service)
