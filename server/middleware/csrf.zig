...
/// Maximum tokens per session to prevent memory exhaustion
...
pub const CsrfStore = struct {
    ...
    pub fn generateToken(self: *CsrfStore, session_key: []const u8) ![]const u8 {
        ...
        // Count existing tokens for this session and remove oldest if at limit
        var session_tokens = std.ArrayList([]const u8){};
        defer session_tokens.deinit(self.allocator);

        var iter = self.tokens.iterator();
        while (iter.next()) |entry| {
            if (std.mem.eql(u8, entry.value_ptr.session_key, session_key)) {
                try session_tokens.append(self.allocator, entry.key_ptr.*);
            }
        }

        // If at limit, remove oldest token(s)
        if (session_tokens.items.len >= MAX_TOKENS_PER_SESSION) {
            // Sort by expiration time to find oldest
            var oldest_key: ?[]const u8 = null;
            var oldest_expiry: i64 = std.math.maxInt(i64);

            for (session_tokens.items) |key| {
                if (self.tokens.get(key)) |csrf_token| {
                    if (csrf_token.expires_at < oldest_expiry) {
                        oldest_expiry = csrf_token.expires_at;
                        oldest_key = key;
                    }
                }
            }
