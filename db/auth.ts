 ...
 *
 * SQL operations for users, auth_sessions, and email_verification_tokens.
 */
...
export async function getUserByActivationToken(token: string) {
  ...
  const [tokenData] = await sql`
    ...
    WHERE t.token_hash = ${tokenHash} AND t.token_type = 'activate' AND t.expires_at > NOW()
...
export async function getUserByResetToken(token: string) {
  ...
  const [tokenData] = await sql`
    ...
    WHERE token_hash = ${tokenHash} AND token_type = 'reset_password' AND expires_at > NOW()
  `;
...
export async function createUser(userData: {
  ...
  // Hash the activation token before storing (only store the hash, never the plain token)
  const tokenHash = hashToken(userData.activationToken);

  // Create activation token
  const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
  await sql`
    INSERT INTO email_verification_tokens (user_id, email, token_hash, token_type, expires_at)
    VALUES (${user.id}, ${userData.email}, ${tokenHash}, 'activate', ${expiresAt})
  `;
...
export async function activateUser(token: string) {
  ...
  const [tokenData] = await sql`
    ...
    FROM email_verification_tokens
    WHERE token_hash = ${tokenHash} AND token_type = 'activate' AND expires_at > NOW()
  `;
  ...
  await sql`
    ...
    WHERE token_hash = ${tokenHash} AND token_type = 'activate'
  `;
...
export async function createPasswordResetToken(userId: number, token: string, expiresAt: Date) {
  ...
  await sql`
    INSERT INTO email_verification_tokens (user_id, email, token_hash, token_type, expires_at)
    VALUES (${userId}, ${user.email}, ${tokenHash}, 'reset_password', ${expiresAt})
...
export async function deletePasswordResetToken(token: string) {
  ...
  await sql`
    DELETE FROM email_verification_tokens
    WHERE token_hash = ${tokenHash} AND token_type = 'reset_password'
  `;
}
