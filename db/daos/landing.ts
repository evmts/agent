...
// =============================================================================
// Extended Landing Request Operations with Joins
// =============================================================================

export interface LandingRequestWithAuthor extends LandingRequest {
  authorUsername?: string;
}

/**
 * List landing requests with author usernames, sorted by status priority
 */
export async function listWithAuthors(
  repositoryId: number
): Promise<LandingRequestWithAuthor[]> {
  const rows = await sql`
    SELECT lq.*, u.username as author_username
    FROM landing_queue lq
    LEFT JOIN users u ON lq.author_id = u.id
    WHERE lq.repository_id = ${repositoryId}
    ORDER BY
      CASE
...
export async function getCommitStatuses(
  ...
  const rows = await sql`
    SELECT id, context, state, description, target_url, workflow_run_id
  ...
  return rows.map(row => ({
    ...
    workflowRunId: row.workflow_run_id ? Number(row.workflow_run_id) : null,
  }));
}

/**
 * Get simple commit status states for multiple commits (for status badges)
 */
export async function getCommitStatusStates(
  repositoryId: number,
  commitSha: string
